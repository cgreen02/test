...
document.querySelectorAll(".productOptionSlideLabelInput").forEach(function (element) {
    element.addEventListener("change", function (e) {
        ...
        // check if all the inputs are checked
        if (document.querySelectorAll(".productOptionSlide").length === document.querySelectorAll(".productOptionSlideLabelInput:checked").length) {
        ...
        let options = {};
        document.querySelectorAll(`.productOptionSlideLabelInput:checked`).forEach(element => {
            options[element.getAttribute("name")] = element.value;
        });

        document.querySelector(".productPair").setAttribute("options", JSON.stringify(options));
        document.querySelector(".productPair").classList.add("active");
        updateProduct();
    });
})
...
function updateProduct() {
    ...
    document.querySelectorAll(".productPair.active").forEach((element, index) => {
        // reindex
        element.querySelector(".productPairTitle").textContent = `PAIR ${index + 1}`
